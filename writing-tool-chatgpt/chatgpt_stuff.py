...
class Chatbot:
    ...
    (most important one is `talk_to_bot()`)
        ...
        - use_api(): Returns the next 'assistant' message.
...
def simple_loop(memory=None):
    ...
    while True:
        ...
        if command == 'api':
            ...
            last_item = cb.messages[-1]
            if last_item['role'] == 'assistant':
                tts_handler.play_tts(tts_handler.get_tts(last_item['content']))
            else:
                print('last item is not from assistant')
        elif command == 'back':
            cb.go_back()
...
def get_cacher_handler(key):
    assert key in ['tts', 'whisper', 'youtube', 'chatgpt'], 'invalid key'
    assert key in ['tts', 'chatgpt'], 'rest not implemented yet lol'

    # get setup info
    with open('setup_info.json', 'r') as f:
        setup_info = json.load(f)
    # initialize cacher and handler
    if key == 'tts':
        cacher = Cache(setup_info['json files']['tts'])
        handler = TtsHandler(cacher, setup_info['output folders']['tts'])
    elif key == 'whisper':
        cacher = Cache(setup_info['json files']['whisper'])
        handler = WhisperHandler(cacher, setup_info['output folders']['whisper'])
    elif key == 'youtube':
        cacher = Cache(setup_info['json files']['youtube'])
        handler = YoutubeHandler(cacher, setup_info['output folders']['youtube'])
    elif key == 'chatgpt':
        cacher = Cache(setup_info['json files']['chatgpt'])
        handler = ChatgptHandler(cacher, setup_info['output folders']['chatgpt'])

    return cacher, handler
